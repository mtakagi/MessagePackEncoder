...
class MapTests : XCTestCase {
    ...
    func testDecodeEmptyMap() {
        let decoder = MessagePackDecoder()
        let map : [String:String] = [:]
        let result = try! decoder.decode([String:String].self, from: Data([0x80]))

        XCTAssertEqual(result!, map)
    }
    ...
    func testDecodeStringMap() {
        let decoder = MessagePackDecoder()
        let map = ["foo" : "hoge", "bar" : "fuga", "bazz" : "piyo"]
        let result = try! decoder.decode([String:String].self,
                                         from: Data([0x83, 0xa4, 0x62, 0x61, 0x7a, 0x7a, 0xa4, 0x70, 0x69, 0x79, 0x6f, 0xa3, 0x62, 0x61, 0x72, 0xa4, 0x66, 0x75, 0x67, 0x61, 0xa3, 0x66, 0x6f, 0x6f, 0xa4, 0x68, 0x6f, 0x67, 0x65]))
    ...
    func testDecodeBoolMap() {
        let decoder = MessagePackDecoder()
        let map = ["true" : true, "false" : false]
        let result = try! decoder.decode([String:Bool].self,
                                         from: Data([0x82, 0xa4, 0x74, 0x72, 0x75, 0x65, 0xc3, 0xa5, 0x66, 0x61, 0x6c, 0x73, 0x65, 0xc2]))
    ...
    static var allTests = [
        ...
        ("testDecodeEmptyMap", testDecodeEmptyMap),
        ("testEncodeStringMap", testEncodeStringMap),
        ("testDecodeStringMap", testDecodeStringMap),
        ("testEncodeBoolMap", testEncodeBoolMap),
        ("testDecodeBoolMap", testDecodeBoolMap),
    ]
}
